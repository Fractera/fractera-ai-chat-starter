...
// 🛑 ЭТОТ ФАЙЛ НИКЕМ НЕ ЗОВЁТСЯ И ЯВЛЯЕТСЯ ПАРКОВКОЙ, А НЕ ЖИВЫМ КОДОМ.
//
// Решение владельца 2026-09-03: «сейчас мы не концентрируемся на том, как это
// будет работать с точки зрения ответа… ещё очень рано. Я тебе дал конкретное
// задание — обеспечить приём сообщений от разных агентов».
//
// Написан раньше времени и отключён от двери в тот же час. Оставлен, потому что
// удалять доказанную работу дороже, чем пометить её; включать — отдельным
// подшагом и по слову владельца, НЕ «раз уж лежит».
//

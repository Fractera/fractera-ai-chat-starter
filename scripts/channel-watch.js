...
//   2) ЖИВОСТЬ ОПРАШИВАТЕЛЯ (шаг 120): заметить, что бот перестал ЧИТАТЬ
//      сообщения, перезапустить канал и сказать об этом. Признак ОДИН — очередь
//      Telegram; второй, по сокетам, оказался ложным и убран (125, см. надгробие).
//
...
// 🪦 ВТОРОЙ ПРИЗНАК — «ОПРАШИВАТЕЛЬ ЖИВ, А СОКЕТОВ НЕТ» — УБРАН 2026-09-05 (125),
// И УБРАН ЦЕЛИКОМ, А НЕ ПОЧИНЕН. Он считал соединения с Telegram по IPv4-адресам
// (`149.154`, `91.108`) и объявлял смерть, когда их не видел.
//
// ✗ ПРИЗНАК ОКАЗАЛСЯ ЛОЖНЫМ, И НАШЁЛ ЭТО САМ АГЕНТ В TELEGRAM. У сервера есть
// IPv6, `api.telegram.org` резолвится в `2001:67c:4e8:f004::9`, и resolver
// отдаёт его ПЕРВЫМ. Когда бот подключался по IPv6, регулярка соединения не
// узнавала — и сторож объявлял мёртвым исправный канал.
// ✗ ЦЕНА, ИЗМЕРЕННАЯ: 2 ложные тревоги, 8 перезапусков канала за 50 минут,
// две телеграммы владельцу о поломке, которой не было. Настоящий отказ за всё
// время был ОДИН, и его поймал первый признак — очередь.
//
// 🔒 ПОЧЕМУ НЕ ПОЧИНЕН ДОБАВЛЕНИЕМ IPv6, А ВЫБРОШЕН. Список адресов Telegram —
// это рукописный список, который расходится с миром молча: сегодня IPv6,
// завтра новый диапазон, послезавтра прокси. Признак, требующий знать ЧУЖИЕ
// адреса, ошибается в ту сторону, где его ошибку принимают за поломку.
// Очередь (`pending_update_count`) не требует знать ничего: Telegram сам
// говорит, сколько сообщений НИКТО не забрал.
//
// 🛑 ЦЕНА ОТКАЗА НАЗВАНА: смерть опрашивателя В ТИШИНЕ теперь не замечается,
// пока владелец не напишет. Это приемлемо ровно потому, что безвредно: смерть
// без входящих сообщений никому не мешает, а первое же сообщение поднимает
// очередь, и канал чинится за две минуты — доказано живьём 2026-09-05 в 12:25.
// 🔒 ЛУЧШЕ РЕДКАЯ ЗАДЕРЖКА, ЧЕМ ЧАСТАЯ ЛОЖНАЯ ТРЕВОГА: тревога, которая врёт,

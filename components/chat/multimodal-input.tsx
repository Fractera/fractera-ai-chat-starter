...
import { AutomationModeSelector } from "@/components/chat/automation-mode-selector";
...
// 🪦 ЗДЕСЬ БЫЛ `setCookie` — ЕДИНСТВЕННЫЙ ЕГО ПОТРЕБИТЕЛЬ ЗАПИСЫВАЛ КУКУ
// `chat-model`, и вместе с выбором модели (112-2) он остался без работы.
// 🔒 Режим автоматизации куки не заводит намеренно: он принадлежит ПРОЕКТУ, а не
// зрителю, и живёт в `PLATFORM-CONFIG` — иначе два человека увидели бы разные
      ...
      >
        ...
        <PromptInputFooter className="px-3 pb-3">
          <PromptInputTools>
            ...
            />
            {/* 🪦 ЗДЕСЬ СТОЯЛ ВЫБОР МОДЕЛИ — заменён переключателем стратегии
                автоматизации (112-2, 2026-09-04). Причина в самом компоненте:
                модель становится персональной у каждого агента, и один выбор на
                весь конвейер обещал бы власть, которой у него нет. */}
...
// 🪦 ЗДЕСЬ ЖИЛИ `ModelSelectorOption` И `ModelSelectorCompact` — ВЫПАДАЮЩИЙ СПИСОК
// МОДЕЛЕЙ OpenAI В ПОЛЕ ВВОДА. Удалены 2026-09-04 (112-2) прямым словом владельца:
// «внутри чата переключатель модели нецелесообразно использовать — один выбор ко
// всем агентам… переходим к персональным настройкам каждого агента».
//
// 🔒 УДАЛЕНЫ, А НЕ ОСТАВЛЕНЫ БЕЗ ПОТРЕБИТЕЛЯ. Компонент, которого никто не
// вызывает, следующая сессия читает как забытый, а не как отменённый: его чинят,
// переносят и однажды монтируют обратно. Восстанавливается из git одной командой —
// `git log --diff-filter=M -1 -- components/chat/multimodal-input.tsx`.
//
// 🔒 ЧТО НЕ УДАЛЕНО И ПОЧЕМУ: примитивы `ModelSelector*` вендоренной библиотеки
// живы — на них построен новый переключатель стратегии, и `/api/models` тоже жив:
// признак `vision` оттуда по-прежнему гасит скрепку вложений.

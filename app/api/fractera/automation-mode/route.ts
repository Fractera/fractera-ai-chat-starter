...
export async function POST(request: Request) {
  ...
  // 🔒 ЗНАЧЕНИЕ ПРОВЕРЯЕТСЯ ЗДЕСЬ, А НЕ ТОЛЬКО СХЕМОЙ НА ТОЙ СТОРОНЕ. Схема
  // отбрасывает незаявленный ключ МОЛЧА: без этой проверки мусорное значение
  // вернуло бы `200`, а на диск не легло бы ничего — «сохранено» стало бы ложью.

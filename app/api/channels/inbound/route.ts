...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 🔒 ОТВЕТ ВКЛЮЧЁН ОБРАТНО ПО ПРЯМОМУ СЛОВУ ВЛАДЕЛЬЦА 2026-09-03, ДОСЛОВНО:
    // «когда сообщения приходят из телеграма, искусственный интеллект не
    // отвечает… почему-то сообщения из телеграма не попадают в запрос к
    // искусственному интеллекту, а нужно бы».
    //
    // 🪦 РАНЬШЕ В ЭТОМ МЕСТЕ СТОЯЛ ЗАПРЕТ, И ОН БЫЛ ЕГО ЖЕ РЕШЕНИЕМ: «переключаться
    // на решение ответов ещё очень рано». Тогда задачей был ПРИЁМ, и он не был
    // закончен. Теперь приём работает — условие отпало, и запрет отменён тем же
    // голосом, что его поставил. Надгробие оставлено намеренно: без него
    // следующая сессия воскресит запрет по памяти и снимет ответ снова.
    //
    // 🛑 ПОВТОР НЕ ОТВЕЧАЕТ ВТОРОЙ РАЗ. Служба повторяет доставку, если мы не
    // успели ответить вовремя; без этого условия каждый повтор порождал бы новый
    // ответ на уже отвеченное.
    //
